...
import { MelodyData, Scale } from '../../interfaces/melody-model';
...
export class ScoreComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit(): void {
    if (this.melodyData) {
      this.setScoreData(this.melodyData);
      return;
    }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['melodyData']?.currentValue) {
      this.setScoreData(changes['melodyData'].currentValue);
    }
  }

  private setScoreData(data: MelodyData): void {
    if (
      data.melody === this.composedMelody &&
  ...
  createMeasures(
    ...
    const staveMeasure3 = new Stave(
      staveMeasure2.getWidth() + staveMeasure2.getX(),
      0,
      300,
    );
    const staveMeasure4 = new Stave(
      staveMeasure3.getWidth() + staveMeasure3.getX(),
      0,
      300,
    );
    const staveMeasure5 = new Stave(0, 0, 300);
    const staveMeasure6 = new Stave(
      staveMeasure5.getWidth() + staveMeasure5.getX(),
      0,
      300,
    );
    const staveMeasure7 = new Stave(
      staveMeasure6.getWidth() + staveMeasure6.getX(),
      0,
      300,
    );
    const staveMeasure8 = new Stave(
      staveMeasure7.getWidth() + staveMeasure7.getX(),
      0,
      300,
    );
    ...
    if (this.checkForScalesWithoutSign(settings.scale))
      keySignature.addToStave(staveMeasure1);
        ...
        if (!this.checkForScalesWithoutSign(settings.scale)) {
          ...
          if (sign.length > 0) {
            ...
            if (index > 0 && this.composedMelody[index - 1].note.length === 3) {
              let lastNote = this.removeMiddleChar(
                this.composedMelody[index - 1].note,
              );
              if (note === lastNote) {
                const n2 = new StaveNote({
                  clef: 'treble',
                  keys: [keys],
                  duration: duration,
                  auto_stem: true,
  ...
  private getAccidentalForMinorExceptions(
    note: string,
    rootKey: string,
    scale: string,
  ): string {
    ...
    const letter = note[0].toUpperCase();
    const targetLetters = this.getRaisedDegreeLettersForScale(
      this.melodySettings.key,
      scale,
    );
  ...
  private getRaisedDegreeLettersForScale(
    tonicKey: string,
    scale: string,
  ): Set<string> {
