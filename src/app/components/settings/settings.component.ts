...
export class SettingsComponent {
  ...
  private readonly CROSS_KEYS = new Set(['G', 'D', 'A', 'E', 'B', 'F#']);
  ...
  constructor(
    ...
    effect(() => {
      const isAuthInitialized = this.authService.isAuthInitialized();
  ...
  ngOnInit(): void {
    const currentSettings = this.creationService.getSettings();
  ...
  private applyRestrictions(): void {
    if (this.hasRestrictions) {
      ...
      this.bars = this.userIsAuthenticated
        ? this.restrictedBars
        : this.unauthorizedBars;
    ...
    this.userService.checkCreditsAvailable(1).subscribe({
      next: (response: {
        ...
        if (response.hasEnoughCredits) {
          ...
          this.isLoading = false;
          this.toastr.error(
            `Cannot create melody: ${response.message || 'Insufficient credits'}`,
          );
        }
        this.cdr.markForCheck();
      },
  ...
  private createMelody() {
    this.melodySaved = false;
    this.creationService
      .submitSettings(this.settings, this.userIsAuthenticated)
      .subscribe({
        next: (result) => {
  ...
  save() {
        ...
        error: (error) => {
          ...
          if (error.status === 403) {
            const errorMsg =
              ...
              error.error.message ||
              'Cannot save melody with current settings';
    ...
    const dialogRef = this.dialog.open(MatModalComponent, {
      data: {
        title: 'Save Melody',
        message:
          'This melody was created before you logged in. You can save it for 1 credit.',
      },
    });
      ...
      this.userService.checkCreditsAvailable(1).subscribe({
        next: (response: {
          ...
          if (response.hasEnoughCredits) {
            ...
            // Insufficient credits
            this.toastr.error(
              `Cannot save melody: ${response.message || 'Insufficient credits'}`,
            );
          }
        },
        error: (error: any) => {
  ...
  checkForEnharmonicConfusion(settings: Settings): string {
    if (this.isMode(settings.scale)) {
      ...
      if (hasFlat && hasCrossKey) {
        ...
        return enharmonic
          ? `${enharmonic} (enharm. ${settings.key})`
          : settings.key;
      }
    }
    return settings.key;
  ...
  private persistSettings(): void {
    this.creationService.updateSettings(this.settings);
  }

  resetSettings(): void {
    this.settings = this.creationService.resetSettings();
    this.melodyDescription = this.setDescription(this.settings);
  }
  ...
  private trackUnauthorizedMelodyCreation(): void {
    ...
    try {
      const currentCount = parseInt(
        localStorage.getItem(this.UNAUTHORIZED_MELODY_COUNT_KEY) || '0',
      );
      const newCount = currentCount + 1;
      localStorage.setItem(
        this.UNAUTHORIZED_MELODY_COUNT_KEY,
        newCount.toString(),
      );
